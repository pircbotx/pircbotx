...
public class InputParser implements Closeable {
	...
	public void processConnect(String rawLine, String code, String target, List<String> parsedLine) throws IrcException, IOException {
		if (CONNECT_CODES.contains(code)) {
			...
			bot.onLoggedIn(configuration.getName() + (nickSuffix == 0 ? "" : nickSuffix));
	...
	public void processCommand(String target, UserHostmask source, String command, String line, List<String> parsedLine) throws IOException {
		...
		if (command.equals("PRIVMSG") && message.startsWith("\u0001") && message.endsWith("\u0001")) {
			...
			if (request.equals("VERSION"))
				// VERSION request
				configuration.getListenerManager().dispatchEvent(new VersionEvent(bot, source, sourceUser, channel));
			else if (request.startsWith("ACTION "))
				// ACTION request
				configuration.getListenerManager().dispatchEvent(new ActionEvent(bot, source, sourceUser, channel, target, request.substring(7)));
			else if (request.startsWith("PING "))
				// PING request
				configuration.getListenerManager().dispatchEvent(new PingEvent(bot, source, sourceUser, channel, request.substring(5)));
			else if (request.equals("TIME"))
				// TIME request
				configuration.getListenerManager().dispatchEvent(new TimeEvent(bot, channel, source, sourceUser));
			else if (request.equals("FINGER"))
				// FINGER request
				configuration.getListenerManager().dispatchEvent(new FingerEvent(bot, source, sourceUser, channel));
